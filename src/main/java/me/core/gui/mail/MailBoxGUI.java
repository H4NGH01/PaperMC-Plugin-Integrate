...
public class MailBoxGUI extends MultiplePageGUI {
    ...
    private @NotNull InventoryItem info(Player p) {
        InventoryItem item = new InventoryItem(Material.PAPER).addTag("ItemTag", "gui.mail.box.info");
        item.setDisplayName(Component.translatable("gui.mail.box.info"));
        item.addLore(ComponentUtil.component(ComponentUtil.translate("gui.mail.box.info_total"), Component.text(ChatColor.YELLOW.toString() + plugin.getMailManager().getMailCount(p))));
        item.addLore(ComponentUtil.component(ComponentUtil.translate("gui.mail.box.info_new"), Component.text(ChatColor.YELLOW.toString() + plugin.getMailManager().getUnreadMail(p).size())));
        return item;
    }

    @Contract(" -> new")
    private @NotNull InventoryItem writeMail() {
        InventoryItem item = new InventoryItem(Material.WRITABLE_BOOK).addTag("ItemTag", "gui.mail.box.write");
    ...
    private @NotNull InventoryItem sentMail() {
        InventoryItem item = new InventoryItem(Material.WRITTEN_BOOK).addTag("ItemTag", "gui.mail.box.sent");
    ...
    private @NotNull InventoryItem mailBin() {
        InventoryItem item = new InventoryItem(Material.BUCKET).addTag("ItemTag", "gui.mail.box.bin");
    ...
    private @NotNull InventoryItem deleteMail() {
        InventoryItem item = new InventoryItem(Material.CAULDRON).addTag("ItemTag", "gui.mail.box.delete");
    ...
    private @NotNull ItemStack mail(@NotNull Mail mail) {
        InventoryItem item = new InventoryItem(Material.PAPER).addTag("ItemTag", "gui.mail.box.mail").addTag("MailID", mail.getMailID());
