...
public class MailBinGUI extends MultiplePageGUI {
    ...
    private @NotNull InventoryItem info(Player p) {
        InventoryItem item = new InventoryItem(Material.PAPER).addTag("ItemTag", "gui.mail.bin.info");
        item.setDisplayName(ComponentUtil.translate("gui.mail.bin.info"));
        item.setLore(ComponentUtil.component(ComponentUtil.translate("gui.mail.bin.info_count"), Component.text(ChatColor.YELLOW.toString() + plugin.getMailManager().getDeletedMail(p).size())));
        return item;
    }

    private @NotNull ItemStack mail(@NotNull Mail mail) {
        InventoryItem item = new InventoryItem(Material.MAP).addTag("ItemTag", "gui.mail.bin.mail").addTag("MailID", mail.getMailID());
